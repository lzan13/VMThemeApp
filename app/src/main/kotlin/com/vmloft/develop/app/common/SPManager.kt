...
object SPManager {
    ...
    // 引导版本
    private val guideVersionKey = "guideVersionKey"

    // 隐私协议状态
    private val policyVersionKey = "policyVersionKey"
    ...
    /**
     * 引导界面，这里根据本地记录的 appVersion 以及运行 APP 获取到的 appVersion 对比
     */
    fun isNeedShowGuide(): Boolean {
        // 上次运行保存的版本号
        val currentVersion = get(settingsEntry, guideVersionKey, 0) as Int
        return currentVersion < Constants.guideVersion
    }

    fun setGuidesDone() {
        putAsync(settingsEntry, guideVersionKey, Constants.policyVersion)
    }

    /**
     * 协议与政策状态
     */
    fun isNeedShowPolicy(): Boolean {
        val policyVersion = get(settingsEntry, policyVersionKey, 0) as Int
        return policyVersion < Constants.policyVersion
    }

    fun setAgreementPolicyDone() {
        putAsync(settingsEntry, policyVersionKey, Constants.policyVersion)
    }
